...
def setup_routes(app: FastAPI):
    ...
    async def health_check(detailed: bool = False):
        ...
        service_manager = (
            app.state.service_manager if hasattr(app.state, "service_manager") else None
        )
            ...
            health_data.update(
                {
                    ...
                    "last_error_time": (
                        service_health.last_error_time.isoformat()
                        if service_health.last_error_time
                        else None
                    ),
                }
            )
...
def setup_exception_handlers(app: FastAPI):
    ...
    async def global_exception_handler(request: Request, exc: Exception):
        ...
        try:
            metrics = get_metrics_collector()
            await metrics.record_request(
                method=request.method, path=str(request.url.path), status_code=500, duration_ms=0.0
            )
        except Exception:
...
async def main():
    ...
    # Prefer env (pydantic nested models may not get env in Docker)
    port = int(os.environ.get("SERVICE_PORT", settings.service.port))
    host = os.environ.get("SERVICE_HOST", settings.service.host)
    # Configure uvicorn
    ...
    try:
        logger.info(f"🚀 Starting Kite Services on {host}:{port}")
        await server.serve()
