...
async def get_market_data(request: MarketDataRequest):
    ...
    try:
        ...
        if request.data_type == "quote":
            # Kite quote API expects "EXCHANGE:SYMBOL" format; normalize if missing
            exchange_prefix = f"{request.exchange.value}:"
            for symbol in request.symbols:
                try:
                    kite_symbol = symbol if ":" in symbol else f"{exchange_prefix}{symbol}"
                    quote = await kite_client.quote([kite_symbol])
...
async def get_quotes(request: RealTimeRequest):
    ...
    try:
        ...
        # Kite expects "EXCHANGE:SYMBOL"; normalize if missing
        exchange_prefix = f"{request.exchange.value}:"
        kite_symbols = [s if ":" in s else f"{exchange_prefix}{s}" for s in request.symbols]

        # Get quotes from Kite
        quotes = await kite_client.quote(kite_symbols)
        stocks_data = []
        failed_symbols = []

        for symbol, kite_symbol in zip(request.symbols, kite_symbols):
            try:
                if kite_symbol in quotes and quotes[kite_symbol]:
                    ...
                    stock_data = {
                        ...
                        "close_price": (
                            Decimal(str(quote_data.get("ohlc", {}).get("close", 0)))
                            if quote_data.get("ohlc", {}).get("close")
                            else None
                        ),
                        "change": (
                        ...
                        "change_percent": (
                            Decimal(str(quote_data.get("net_change_percent", 0)))
                            if quote_data.get("net_change_percent") is not None
                            else None
                        ),
