...
from models.unified_api_models import (
    MarketDataRequest, MarketDataResponse, StockData,
    MarketStatusResponse, MarketStatus,
    InstrumentsResponse, InstrumentInfo,
    Exchange, Interval
)
...
async def get_instruments(
    ...
    try:
        ...
        for tradingsymbol, instrument in instruments_data.items():
            ...
            filtered_instruments.append(InstrumentInfo(
...
async def get_quotes(request: RealTimeRequest):
    ...
    try:
            ...
            try:
                if symbol in quotes and quotes[symbol]:
                    ...
                    stock_data = {
                        ...
                        "close_price": Decimal(str(quote_data.get("ohlc", {}).get("close", 0))) if quote_data.get("ohlc", {}).get("close") else None,
                        "change": Decimal(str(quote_data.get("net_change", 0))) if quote_data.get("net_change") is not None else None,
                        "change_percent": Decimal(str(quote_data.get("net_change_percent", 0))) if quote_data.get("net_change_percent") is not None else None,
                        "volume": quote_data.get("volume", 0),
                        "average_price": Decimal(str(quote_data.get("average_price", 0))) if quote_data.get("average_price") else None,
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Quotes fetch failed: {str(e)}"
        )
