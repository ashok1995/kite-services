...
from models.enhanced_context_models import VolatilityLevel
...
class QuickOpportunityRequest(BaseModel):
    ...
    timeframe: str = Field(
        default="5minute",
        description="Timeframe: 5minute, 15minute, 1hour"
    )
    opportunity_types: List[str] = Field(
        default=["breakout", "reversal", "momentum"],
        description="Types of opportunities to find"
    )
...
async def get_quick_opportunities(request: QuickOpportunityRequest):
    ...
    try:
        ...
        if not nifty_quote:
            raise HTTPException(
                status_code=503,
                detail="Unable to fetch Nifty data from Kite"
            )
        ...
        # Calculate distances
        for level, level_type in [(s1, "support"), (s2, "support"), (r1, "resistance"), (r2, "resistance")]:
            distance_pct = abs((level - current_price) / current_price * 100).quantize(Decimal("0.01"))
        ...
        if "breakout" in request.opportunity_types:
            ...
            if immediate_resistance and nifty_5min_trend == "bullish":
                ...
                if nearest_resistance.distance_percent < Decimal("0.3"):
                    target = nearest_resistance.level + (nearest_resistance.level * Decimal("0.005"))  # 0.5% above
                    stop_loss = current_price - (current_price * Decimal("0.003"))  # 0.3% below
                    risk_reward = float((target - current_price) / (current_price - stop_loss)) if (current_price - stop_loss) > 0 else 1.0
                    ...
                    opportunities.append(OpportunitySignal(
                        ...
                        reasoning=f"Near resistance at {nearest_resistance.level}, bullish momentum, position for breakout",
                        time_sensitive=True,
        ...
        if "reversal" in request.opportunity_types:
            ...
            if rsi_5min > 70 and immediate_resistance:
                ...
                if nearest_resistance.distance_percent < Decimal("0.2"):
                    ...
                    stop_loss = nearest_resistance.level + (nearest_resistance.level * Decimal("0.002"))  # 0.2% above resistance
                    risk_reward = float((current_price - target) / (stop_loss - current_price)) if (stop_loss - current_price) > 0 else 1.0
                    ...
                    opportunities.append(OpportunitySignal(
                        ...
                        stop_loss=stop_loss.quantize(Decimal("0.01")),
                        risk_reward_ratio=round(risk_reward, 2),
                        reasoning=f"Overbought (RSI {rsi_5min:.1f}), near resistance, expect reversal",
                        time_sensitive=True,
            ...
            # Oversold reversal (RSI < 30 near support)
                ...
                if nearest_support.distance_percent < Decimal("0.2"):
                    ...
                    stop_loss = nearest_support.level - (nearest_support.level * Decimal("0.002"))  # 0.2% below support
                    risk_reward = float((target - current_price) / (current_price - stop_loss)) if (current_price - stop_loss) > 0 else 1.0
        ...
        if "momentum" in request.opportunity_types:
            ...
            if abs(change_pct) > Decimal("0.3") and volume_surge:
                if change_pct > 0:  # Bullish momentum
                    ...
                    signal = "buy"
                    reasoning = f"Strong bullish momentum ({change_pct:.2f}%), volume surge detected"
                    ...
                    signal = "sell"
                    reasoning = f"Strong bearish momentum ({change_pct:.2f}%), volume surge detected"
                
                risk_reward = float(abs((target - current_price) / (current_price - stop_loss))) if (current_price - stop_loss) != 0 else 1.0
        ...
        macd_signal = "bullish" if change_pct > 0 and market_momentum == "accelerating" else \
                     "bearish" if change_pct < 0 and market_momentum == "decelerating" else "neutral"
        
        message = f"Found {len(opportunities)} quick trading opportunities"
        if opportunities:
            best_opp = opportunities[0]
            message += f" | Best: {best_opp.opportunity_type} {best_opp.signal} (confidence {best_opp.confidence}%)"
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Quick opportunity scan failed: {str(e)}"
        )
