    ...
    **Response Structure:**
    ...
    try:
        ...
        if request.include_primary:
            try:
                cache_key = _get_cache_key_primary()
        ...
        if request.include_detailed:
            try:
                cache_key = _get_cache_key_detailed()
                ...
                if not detailed_context:
                    ...
                    if cache_service and cache_service.enabled and detailed_context:
                        await cache_service.set(
                            cache_key,
                            detailed_context.dict(),
                            ttl=CacheTTL.DETAILED_CONTEXT
                        )
                        logger.info(f"💾 Cached detailed context (TTL={CacheTTL.DETAILED_CONTEXT}s)")
        ...
        if request.include_style_specific:
            ...
            if TradingStyle.INTRADAY in request.trading_styles or TradingStyle.ALL in request.trading_styles:
                try:
                    cache_key = _get_cache_key_intraday()
                    ...
                    if not intraday_context:
                        ...
                        if cache_service and cache_service.enabled and intraday_context:
                            await cache_service.set(
                                ...
                                ttl=CacheTTL.COMPOSITE_INTRADAY
                            )
                            logger.info(f"💾 Cached intraday context (TTL={CacheTTL.COMPOSITE_INTRADAY}s)")
            ...
            if TradingStyle.SWING in request.trading_styles or TradingStyle.ALL in request.trading_styles:
                try:
                    cache_key = _get_cache_key_swing()
                    ...
                    if not swing_context:
                        ...
                        if not intraday_context and cache_service and cache_service.enabled:
                            ...
                            intraday_cache_key = _get_cache_key_intraday()
                        ...
                        logger.info(f"⚠️  SWING CONTEXT: Cache MISS - generating (intraday_reuse={'YES' if intraday_base else 'NO'})...")
                        ...
                        if cache_service and cache_service.enabled and swing_context:
                            await cache_service.set(
                                cache_key,
                                swing_context.dict(),
                                ttl=CacheTTL.COMPOSITE_SWING
                            )
                            logger.info(f"💾 Cached swing context (TTL={CacheTTL.COMPOSITE_SWING}s)")
            ...
            if TradingStyle.LONG_TERM in request.trading_styles or TradingStyle.ALL in request.trading_styles:
                try:
                    cache_key = _get_cache_key_longterm()
                    ...
                    if not long_term_context:
                        ...
                        if not swing_context and cache_service and cache_service.enabled:
                            ...
                            swing_cache_key = _get_cache_key_swing()
                        ...
                        logger.info(f"⚠️  LONG-TERM CONTEXT: Cache MISS - generating (swing_reuse={'YES' if swing_base else 'NO'})...")
                        ...
                        if cache_service and cache_service.enabled and long_term_context:
                            await cache_service.set(
                                ...
                                ttl=CacheTTL.COMPOSITE_LONGTERM
                            )
                            logger.info(f"💾 Cached long-term context (TTL={CacheTTL.COMPOSITE_LONGTERM}s)")
        ...
        context_quality_score = _calculate_context_quality(
            primary_context,
            detailed_context,
            intraday_context,
            swing_context,
...
async def _generate_primary_context(market_service, intelligence_service) -> PrimaryMarketContext:
    ...
    try:
        ...
        us_change = Decimal(str(sum(m.change_percent for m in us_markets) / len(us_markets))) if us_markets else Decimal("0")
        asia_change = Decimal(str(sum(m.change_percent for m in asia_markets) / len(asia_markets))) if asia_markets else Decimal("0")
        europe_change = Decimal(str(sum(m.change_percent for m in europe_markets) / len(europe_markets))) if europe_markets else Decimal("0")
        
        # Determine global trend
        avg_global_change = (us_change + asia_change + europe_change) / 3
        overall_trend = "bullish" if avg_global_change > 0.3 else "bearish" if avg_global_change < -0.3 else "neutral"
        ...
        risk_on_off = "risk_on" if avg_global_change > 0 else "risk_off" if avg_global_change < -0.5 else "neutral"
    ...
    try:
        ...
        # Trend direction
        trend_direction = "up" if nifty_change > 0.2 else "down" if nifty_change < -0.2 else "sideways"
    ...
    overall_score = _calculate_market_score(global_context, indian_context)
    
    # Determine favorable trading styles
    favorable_styles = _determine_favorable_styles(overall_score, indian_context.market_regime)
    
    # Calculate confidence based on data availability
    confidence = 0.9 if global_context.us_markets_change != 0 and indian_context.nifty_change != 0 else 0.5
...
async def _generate_detailed_context(
    ...
    try:
        ...
        if distance_to_resistance < Decimal("0.3") and change_percent > Decimal("0.2") and rsi < 70:
            ...
            target_1 = (resistance_5min * Decimal("1.003")).quantize(Decimal("0.01"))  # 0.3% above R
            target_2 = (resistance_5min * Decimal("1.006")).quantize(Decimal("0.01"))  # 0.6% above R
            stop_loss = (pivot_5min * Decimal("0.998")).quantize(Decimal("0.01"))  # 0.2% below pivot
        ...
        # 2. Reversal setup (price at resistance with overbought conditions)
            ...
            stop_loss = (resistance_5min * Decimal("1.002")).quantize(Decimal("0.01"))  # 0.2% above R
        ...
        # 4. Strong momentum scalp (if no setup above but strong momentum)
            ...
            if change_percent > 0:  # Bullish momentum
                ...
                stop_loss = (current_value * Decimal("0.998")).quantize(Decimal("0.01"))  # 0.2% down
                signal = "BUY"
...
async def _generate_intraday_context(
    ...
    try:
        ...
        price_range_percent = ((day_high - day_low) / day_low * 100).quantize(Decimal("0.01")) if day_low > 0 else Decimal("0")
        ...
        price_position = (current_price - day_low) / (day_high - day_low) if (day_high - day_low) > 0 else Decimal("0.5")
...
async def _generate_swing_context(
    ...
    try:
        ...
        if change_percent > 0.5:
            ...
            trend_strength = TrendStrength.STRONG if change_percent > 1.0 else TrendStrength.MODERATE
...
async def _generate_long_term_context(
    market_service,
    intelligence_service,
    swing_base: Optional[SwingContext] = None
) -> LongTermContext:
    """
...
def _calculate_market_score(global_ctx: GlobalMarketPrimary, indian_ctx: IndianMarketPrimary) -> int:
    ...
    if indian_ctx.nifty_change and indian_ctx.nifty_change > 0:
        score += int(float(indian_ctx.nifty_change) * 10)
    elif indian_ctx.nifty_change:
        score += int(float(indian_ctx.nifty_change) * 10)
...
def _determine_favorable_styles(market_score: int, regime: MarketRegime) -> List[TradingStyle]:
    """Determine which trading styles are favorable based on market conditions."""
    
    favorable = []
    
    # Intraday favorable in volatile markets
    if abs(market_score) > 30:
        favorable.append(TradingStyle.INTRADAY)
    
    # Swing favorable in trending markets
    if regime in [MarketRegime.BULL_WEAK, MarketRegime.BULL_STRONG, MarketRegime.BEAR_WEAK]:
        favorable.append(TradingStyle.SWING)
    
    # Long-term favorable in bullish regimes
    if market_score > 20:
...
def _generate_data_quality_report(
    ...
    if detailed_context and detailed_context.nifty_analysis:
        ...
        if detailed_context.nifty_analysis.technicals:
            approximated_count += 1
            approximations.append("Technical indicators: APPROXIMATED (based on current price action)")
            data_sources.append("Technical indicators: APPROXIMATED (need historical data for precise calculation)")
        
        # Sectors
        if detailed_context.sectors and len(detailed_context.sectors) > 0:
            real_count += 1
            data_sources.append(f"Sector data: REAL (Yahoo Finance - {len(detailed_context.sectors)} sectors)")
        else:
            fallback_count += 1
    ...
    if swing_context:
        ...
        approximations.append("Swing trend: Need multi-day historical data for precise trend analysis")
    ...
    data_quality = {
        ...
        "fallbacks": fallbacks,
        "recommendations": _get_data_quality_recommendations(real_count, approximated_count, fallback_count)
    }
...
def _get_data_quality_recommendations(real_count, approximated_count, fallback_count):
    ...
    if approximated_count > 10:
        recommendations.append("MANY APPROXIMATIONS: Consider fetching historical data for better technical indicators")
...
def _extract_ml_features(
    ...
    if primary_context and hasattr(primary_context, 'indian_context') and primary_context.indian_context:
        ...
        features.update({
            ...
            'banknifty_change_pct': float(indian.banknifty_change) if indian.banknifty_change else 0.0,
            'nifty_price': float(indian.current_nifty_price) if indian.current_nifty_price else 0.0,
            ...
            'advance_decline_ratio': float(indian.advance_decline_ratio) if indian.advance_decline_ratio else 1.0,
            'nifty_support': float(indian.nifty_support) if indian.nifty_support else 0.0,
            'nifty_resistance': float(indian.nifty_resistance) if indian.nifty_resistance else 0.0,
        })
    
    # PRIMARY CONTEXT - Global Market
    if primary_context and hasattr(primary_context, 'global_context') and primary_context.global_context:
        global_ctx = primary_context.global_context
        features.update({
            'global_trend': global_ctx.overall_trend if global_ctx.overall_trend else 'unknown',
            'global_trend_strength': global_ctx.trend_strength if global_ctx.trend_strength else 'unknown',
            'us_markets_change': float(global_ctx.us_markets_change) if global_ctx.us_markets_change else 0.0,
            'asia_markets_change': float(global_ctx.asia_markets_change) if global_ctx.asia_markets_change else 0.0,
            'europe_markets_change': float(global_ctx.europe_markets_change) if global_ctx.europe_markets_change else 0.0,
            'risk_sentiment': global_ctx.risk_on_off if global_ctx.risk_on_off else 'neutral',
            'volatility_level': global_ctx.volatility_level if global_ctx.volatility_level else 'normal',
        })
    ...
    if detailed_context:
        ...
        if hasattr(detailed_context, 'nifty_analysis') and detailed_context.nifty_analysis:
            ...
            features.update({
                ...
                'nifty_day_change': float(nifty.change_percent) if nifty.change_percent else 0.0,
            ...
            if tech:
                features.update({
                    ...
                    'nifty_price_vs_sma20': float(tech.price_vs_sma20) if tech.price_vs_sma20 else None,
                    'nifty_price_vs_sma50': float(tech.price_vs_sma50) if tech.price_vs_sma50 else None,
                    'nifty_bollinger_position': tech.bollinger_position if tech.bollinger_position else 'middle',
            ...
            if hasattr(nifty, 'pivot_5min') and nifty.pivot_5min:
                features.update({
                    ...
                    'nifty_support_5min': float(nifty.support_5min) if nifty.support_5min else 0.0,
                    'nifty_resistance_5min': float(nifty.resistance_5min) if nifty.resistance_5min else 0.0,
                })
        ...
        # Market breadth
        features.update({
            'market_advances': int(detailed_context.advances) if detailed_context.advances else 0,
            'market_declines': int(detailed_context.declines) if detailed_context.declines else 0,
            'market_unchanged': int(detailed_context.unchanged) if detailed_context.unchanged else 0,
        })
        
        # Sector performance
        if hasattr(detailed_context, 'sectors') and detailed_context.sectors:
            sector_changes = [float(s.change_percent) for s in detailed_context.sectors if s.change_percent]
            if sector_changes:
                features.update({
                    ...
                    'sector_breadth': sum(1 for c in sector_changes if c > 0) / len(sector_changes),
                })
    
    # INTRADAY CONTEXT
    if intraday_context:
        features.update({
            'intraday_volatility': intraday_context.intraday_volatility if hasattr(intraday_context, 'intraday_volatility') and intraday_context.intraday_volatility else 'unknown',
            'intraday_momentum': intraday_context.current_momentum if hasattr(intraday_context, 'current_momentum') and intraday_context.current_momentum else 'unknown',
            'intraday_trend': intraday_context.market_phase if hasattr(intraday_context, 'market_phase') and intraday_context.market_phase else 'unknown',
    ...
    if swing_context:
        features.update({
            'swing_trend': swing_context.multi_day_trend if hasattr(swing_context, 'multi_day_trend') and swing_context.multi_day_trend else 'unknown',
            'swing_strength': swing_context.trend_strength if hasattr(swing_context, 'trend_strength') and swing_context.trend_strength else 'unknown',
        })

    # LONG-TERM CONTEXT
    if long_term_context:
        features.update({
            'longterm_trend': long_term_context.economic_cycle if hasattr(long_term_context, 'economic_cycle') and long_term_context.economic_cycle else 'unknown',
            'longterm_strength': long_term_context.market_valuation if hasattr(long_term_context, 'market_valuation') and long_term_context.market_valuation else 'unknown',
        })
