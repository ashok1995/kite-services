...
from models.market_context_models import (
    ...
    QuickMarketContextRequest, QuickMarketContextResponse,
    ContextScope, DataFreshness, MarketRegime, VolatilityRegime,
    GlobalMarketSnapshot, IndianMarketSnapshot, TechnicalSnapshot,
    VolatilitySnapshot, SentimentSnapshot, TradingEnvironmentSnapshot,
    MarketInsights, TradingRecommendations, SymbolSpecificContext
)
...
async def get_comprehensive_market_context(
    context_request: MarketContextRequest = Body(...),
    ...
    **This single endpoint replaces 10+ specialized endpoints with unified intelligence!**
    ...
    try:
        ...
        response = await _build_comprehensive_context(
            context_request, 
            services, 
            start_time
        )
        ...
        raise HTTPException(
            status_code=500, 
            detail=f"Market context generation failed: {str(e)}"
        )
...
async def get_quick_market_context(
    ...
    **When you need context NOW, not later!**
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Quick context generation failed: {str(e)}"
        )
...
async def _build_comprehensive_context(
    ...
    global_context = await _build_global_context(
        request, services
    ) if request.include_global_context else _create_empty_global_context()
    
    # 2. Indian market context  
    indian_context = await _build_indian_context(
        request, services
    )
    
    # 3. Technical analysis context
    technical_context = await _build_technical_context(
        request, services
    ) if request.include_technical_analysis else _create_empty_technical_context()
    
    # 4. Volatility context
    volatility_context = await _build_volatility_context(
        request, services
    ) if request.include_volatility_analysis else _create_empty_volatility_context()
    
    # 5. Sentiment context
    sentiment_context = await _build_sentiment_context(
        request, services
    ) if request.include_sentiment_analysis else _create_empty_sentiment_context()
    
    # 6. Trading environment
    trading_environment = await _build_trading_environment(
        request, services
    )
    ...
    if request.symbols and len(request.symbols) > 0:
        symbol_analysis = await _build_symbol_analysis(
            request, services
        )
    ...
    response = MarketContextResponse(
        ...
        risk_opportunity_matrix=_build_risk_opportunity_matrix(
            overall_regime, volatility_context
        ),
...
async def _build_global_context(request: MarketContextRequest, services: dict) -> GlobalMarketSnapshot:
    ...
    try:
        ...
        return GlobalMarketSnapshot(
            timestamp=datetime.now(),
            us_markets={
                "SPX": {"trend": global_trends.sp500_trend, "change": global_trends.overnight_us_change},
                "NASDAQ": {"trend": global_trends.nasdaq_trend, "change": global_trends.overnight_us_change}
            },
            european_markets={
                "FTSE": {"trend": global_trends.ftse_trend, "change": global_trends.overnight_europe_change}
            },
            asian_markets={
                "NIKKEI": {"trend": global_trends.nikkei_trend, "change": global_trends.overnight_asia_change}
            },
            global_sentiment=GlobalSentiment.POSITIVE if global_trends.global_momentum_score > 0 else GlobalSentiment.NEGATIVE,
...
async def _build_indian_context(request: MarketContextRequest, services: dict) -> IndianMarketSnapshot:
