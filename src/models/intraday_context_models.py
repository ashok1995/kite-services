...
class GlobalMarketContext(BaseModel):
    ...
    us_indices: Dict[str, GlobalIndex] = Field(default_factory=dict, description="US market indices")
    european_indices: Dict[str, GlobalIndex] = Field(default_factory=dict, description="European indices")
    ...
    europe_india_correlation: Decimal = Field(..., ge=-1, le=1, description="Europe-India correlation")
    ...
    # Session analysis
    overnight_changes: Dict[str, Decimal] = Field(default_factory=dict, description="Overnight changes")
...
class IntradayMarketContext(BaseModel):
    ...
    indian_indices: Dict[str, GlobalIndex] = Field(default_factory=dict, description="Indian indices")
    ...
    # Multi-timeframe analysis
    timeframe_alignment: Dict[IntradayTimeframe, str] = Field(default_factory=dict, description="Timeframe trends")
    ...
    global_influence_score: Decimal = Field(..., ge=0, le=100, description="Global influence on Indian markets")
    overnight_gap_analysis: Dict[str, Any] = Field(default_factory=dict, description="Overnight gap analysis")
    foreign_flows: Dict[str, Decimal] = Field(default_factory=dict, description="Foreign institutional flows")
...
class IntradayTradingSignal(BaseModel):
    ...
    position_size_percent: Decimal = Field(..., ge=0, le=100, description="Suggested position size %")
    
    # Contextual reasoning
    primary_reasons: List[str] = Field(default_factory=list, description="Primary signal reasons")
    global_factors: List[str] = Field(default_factory=list, description="Global influencing factors")
...
class IntradayContextRequest(BaseModel):
    ...
    include_sector_rotation: bool = Field(default=True, description="Include sector rotation analysis")
    include_multi_timeframe: bool = Field(default=True, description="Include multi-timeframe analysis")
    ...
    position_size_preference: Decimal = Field(default=Decimal("5"), ge=1, le=20, description="Position size %")
...
class IntradayContextResponse(BaseModel):
    ...
    trading_signals: Dict[str, IntradayTradingSignal] = Field(
        ...
        description="Symbol-specific trading signals"
    )
    ...
    overnight_developments: List[str] = Field(default_factory=list, description="Overnight developments")
    
    # Recommendations
    intraday_strategy_suggestions: List[str] = Field(default_factory=list, description="Strategy suggestions")
    risk_management_notes: List[str] = Field(default_factory=list, description="Risk management notes")
...
class GlobalIndexTrend(BaseModel):
    ...
    global_momentum_score: Decimal = Field(..., ge=-100, le=100, description="Global momentum score")
...
class IntradaySectorContext(BaseModel):
    ...
    sector_performance: Dict[str, Decimal] = Field(default_factory=dict, description="Real-time sector performance")
    sector_momentum: Dict[str, str] = Field(default_factory=dict, description="Sector momentum")
    ...
    global_sector_trends: Dict[str, str] = Field(default_factory=dict, description="Global sector trends")
    sector_correlation_with_global: Dict[str, Decimal] = Field(default_factory=dict, description="Global correlation")
    
    # Intraday patterns
    morning_sector_leaders: List[str] = Field(default_factory=list, description="Morning leaders")
    afternoon_sector_leaders: List[str] = Field(default_factory=list, description="Afternoon leaders")
    sector_momentum_shifts: List[str] = Field(default_factory=list, description="Momentum shifts detected")
...
class EnhancedIntradayRequest(BaseModel):
    ...
    include_global_indices: bool = Field(default=True, description="Include global indices analysis")
    include_sector_rotation: bool = Field(default=True, description="Include sector rotation")
    include_multi_timeframe: bool = Field(default=True, description="Include multi-timeframe analysis")
    include_volatility_analysis: bool = Field(default=True, description="Include volatility analysis")
    include_trading_signals: bool = Field(default=True, description="Include trading signals")
    
    # Timeframe preferences
    primary_timeframe: IntradayTimeframe = Field(default=IntradayTimeframe.MINUTE_15, description="Primary timeframe")
    ...
    max_position_size: Decimal = Field(default=Decimal("10"), ge=1, le=25, description="Max position size %")
...
class EnhancedIntradayResponse(BaseModel):
    ...
    # Core context
    intraday_context: IntradayMarketContext = Field(..., description="Comprehensive intraday context")
    global_trends: GlobalIndexTrend = Field(..., description="Global index trends")
    
    # Analysis results
    market_regime_analysis: Dict[str, Any] = Field(default_factory=dict, description="Market regime analysis")
    ...
    global_influence_factors: List[str] = Field(default_factory=list, description="Global influence factors")
