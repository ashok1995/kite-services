...
class RangeLevelAnalysis(BaseModel):
    ...
    # Key levels
    strong_resistance: List[Decimal] = Field(
        default_factory=list, description="Strong resistance levels"
    )
    resistance: List[Decimal] = Field(default_factory=list, description="Resistance levels")
    weak_resistance: List[Decimal] = Field(
        default_factory=list, description="Weak resistance levels"
    )
...
class EnhancedMarketContext(BaseModel):
    ...
    indices: List[Dict[str, Any]] = Field(
        default_factory=list, description="Market indices with trends"
    )
    ...
    # Sector analysis
    sector_context: Dict[str, SectorContext] = Field(
        default_factory=dict, description="Sector-wise context"
    )
...
class StockContextRequest(BaseModel):
    ...
    historical_days: int = Field(
        default=50, ge=10, le=200, description="Historical data for analysis"
    )
...
class StockContextResponse(BaseModel):
    ...
    market_intelligence: Optional[MarketIntelligence] = Field(
        None, description="Market intelligence"
    )
...
class MarketContextRequest(BaseModel):
    ...
    analysis_depth: str = Field(
        default="standard", description="Analysis depth: basic, standard, comprehensive"
    )
...
class MarketContextResponse(BaseModel):
    ...
    stock_contexts: Dict[str, StockContextResponse] = Field(
        default_factory=dict, description="Stock contexts"
    )
