...
class GlobalMarketPrimary(BaseModel):
    ...
    overall_trend: str = Field(
        ..., description="Overall global market trend: bullish/bearish/neutral"
    )
    ...
    europe_markets_change: Optional[Decimal] = Field(
        None, description="European markets avg change %"
    )
...
class IndianMarketPrimary(BaseModel):
    ...
    # Breadth
    advance_decline_ratio: Optional[Decimal] = Field(
        None, description="Advancing vs declining stocks"
    )
...
class PrimaryMarketContext(BaseModel):
    ...
    overall_market_score: int = Field(
        ...
        ge=-100,
        le=100,
        description="Overall market score -100 (very bearish) to +100 (very bullish)",
    )
    market_confidence: float = Field(
        ..., ge=0.0, le=1.0, description="Confidence in market assessment"
    )

    # Trading recommendations
    favorable_for: List[TradingStyle] = Field(
...
class TechnicalIndicators(BaseModel):
    ...
    macd_signal: str = Field(
        ..., description="MACD signal: bullish_crossover/bearish_crossover/neutral"
    )

    # Volatility
    atr_percent: Optional[Decimal] = Field(None, description="ATR as % of price")
    bollinger_position: str = Field(
        ..., description="Price position in Bollinger Bands: upper/middle/lower"
    )
...
class IndexDetailedAnalysis(BaseModel):
    ...
    immediate_resistance: List[Decimal] = Field(
        default_factory=list, description="3 resistance levels"
    )
    ...
    quick_opportunity: Optional[QuickTradingOpportunity] = Field(
        None, description="Quick trading setup for next 5-15 mins"
    )
...
class DetailedMarketContext(BaseModel):
    ...
    sectors: List[SectorPerformance] = Field(
        default_factory=list, description="All sector performances"
    )

    # Top movers
    top_gainers: List[Dict[str, Any]] = Field(
        default_factory=list, description="Top 10 gainers with reasons"
    )
    top_losers: List[Dict[str, Any]] = Field(
        default_factory=list, description="Top 10 losers with reasons"
    )
...
class SwingContext(BaseModel):
    ...
    # Support/Resistance
    swing_support_levels: List[Decimal] = Field(
        default_factory=list, description="Key swing support levels"
    )
    swing_resistance_levels: List[Decimal] = Field(
        default_factory=list, description="Key swing resistance levels"
    )
...
class LongTermContext(BaseModel):
    ...
    # Sector allocation
    recommended_sector_weights: Dict[str, Decimal] = Field(
    ...
    dividend_opportunities: List[str] = Field(
        default_factory=list, description="High dividend yield"
    )
...
class EnhancedMarketContextRequest(BaseModel):
    ...
    include_style_specific: bool = Field(
        default=True, description="Include trading style specific context"
    )

    # Trading style preferences
    trading_styles: List[TradingStyle] = Field(
        default=[TradingStyle.ALL], description="Trading styles to optimize for"
    )

    # Specific symbols for detailed analysis
    focus_symbols: Optional[List[str]] = Field(
        None, max_length=10, description="Specific stocks for detailed analysis"
    )
    ...
    # ML Integration (NEW!)
    include_ml_features: bool = Field(
        default=False, description="Include flattened ML-ready features (47+ features)"
    )
...
class EnhancedMarketContextResponse(BaseModel):
    ...
    contract_version: Optional[str] = Field(
        None, description="Data contract version (e.g., '1.0.0')"
    )
    ...
    ml_features: Optional[Dict[str, Any]] = Field(
        None,
        description="Flattened ML features for pandas/sklearn (47+ numeric & categorical)",
    )

    # DATA QUALITY INFORMATION
    data_quality: Optional[Dict[str, Any]] = Field(
        None, description="Data quality report showing what is real vs approximated vs fallback"
    )
    ...
    data_source_summary: Optional[Dict[str, int]] = Field(
        None, description="Summary: {real: N, approximated: N, fallback: N}"
    )
