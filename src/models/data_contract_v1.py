...
class GlobalMarketContextV1(BaseModel):
    ...
    overall_trend: TrendDirectionV1 = Field(
        ...,
        description="Global market trend direction"
    )
    us_markets_change: float = Field(
        ...,
        ge=-100.0,
        le=100.0,
        description="US markets percentage change"
    )
    asia_markets_change: float = Field(
        ...,
        ge=-100.0,
        le=100.0,
        description="Asia markets percentage change"
    )
    europe_markets_change: float = Field(
        ...,
        ge=-100.0,
        le=100.0,
        description="Europe markets percentage change"
    )
    risk_on_off: Literal["risk_on", "risk_off", "neutral"] = Field(
        ...,
        description="Risk sentiment"
    )
    volatility_level: VolatilityLevelV1 = Field(
        ...,
        description="Global volatility level"
    )
...
class IndianMarketContextV1(BaseModel):
    ...
    nifty_change: float = Field(
        ...,
        ge=-100.0,
        le=100.0,
        description="Nifty 50 percentage change"
    )
    banknifty_change: float = Field(
        ...,
        ge=-100.0,
        le=100.0,
        description="Bank Nifty percentage change"
    )
    sensex_change: float = Field(
        ...,
        ge=-100.0,
        le=100.0,
        description="Sensex percentage change"
    )
    market_regime: MarketRegimeV1 = Field(
        ...,
    ...
    current_nifty_price: Optional[float] = Field(
        None,
        gt=0,
        description="Current Nifty 50 price (INR)"
    )
...
class PrimaryMarketContextV1(BaseModel):
    ...
    market_confidence: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="System confidence in data quality (0.0 to 1.0)"
    )
    favorable_for: List[TradingStyleV1] = Field(
        ...,
        min_length=0,
        max_length=3,
        description="Trading styles favorable in current market"
    )
    global_context: GlobalMarketContextV1 = Field(
        ...,
        description="Global market indicators"
    )
    indian_context: IndianMarketContextV1 = Field(
        ...,
        description="Indian market indicators"
    )
...
class IntradayContextV1(BaseModel):
    ...
    pivot_point: Optional[float] = Field(
        None,
        gt=0,
        description="Daily pivot point price level"
    )
    resistance_1: Optional[float] = Field(
        None,
        gt=0,
        description="First resistance level"
    )
    resistance_2: Optional[float] = Field(
        None,
        gt=0,
        description="Second resistance level"
    )
    resistance_3: Optional[float] = Field(
        None,
        gt=0,
        description="Third resistance level"
    )
    support_1: Optional[float] = Field(
        None,
        gt=0,
        description="First support level"
    )
    support_2: Optional[float] = Field(
        None,
        gt=0,
        description="Second support level"
    )
    support_3: Optional[float] = Field(
        None,
        gt=0,
    ...
    price_vs_vwap: Optional[Literal["above", "below", "at"]] = Field(
        None,
        description="Current price position relative to VWAP"
    )
    ...
    momentum_strength: Optional[TrendStrengthV1] = Field(
        None,
        description="Momentum strength"
    )
    volume_trend: Optional[Literal["high", "average", "low"]] = Field(
        None,
        description="Volume trend relative to average"
    )
    
    # Volatility (IMMUTABLE)
    intraday_volatility: VolatilityLevelV1 = Field(
        ...,
        description="Intraday volatility level"
    )
    expected_range_high: Optional[float] = Field(
        None,
        gt=0,
        description="Expected intraday high"
    )
    expected_range_low: Optional[float] = Field(
        None,
        gt=0,
        description="Expected intraday low"
    )
    
    # Signals (IMMUTABLE)
    breakout_candidates: List[str] = Field(
        default_factory=list,
        description="Stock symbols showing breakout potential"
    )
    reversal_candidates: List[str] = Field(
        default_factory=list,
        description="Stock symbols showing reversal potential"
    )
    high_impact_events_today: List[str] = Field(
        default_factory=list,
        description="High impact events scheduled for today"
    )
...
class SwingContextV1(BaseModel):
    ...
    trend_strength: TrendStrengthV1 = Field(
        ...,
        description="Trend strength"
    )
    trend_age_days: int = Field(
        ...,
        ge=0,
        description="Number of days current trend has persisted"
    )
    weekly_momentum: TrendDirectionV1 = Field(
        ...,
        description="Weekly momentum direction"
    )
    momentum_divergence: bool = Field(
        ...,
        description="Whether momentum divergence detected"
    )
    
    # Support/Resistance (IMMUTABLE)
    swing_support_levels: List[float] = Field(
        default_factory=list,
        description="Swing support price levels"
    )
    swing_resistance_levels: List[float] = Field(
        default_factory=list,
        description="Swing resistance price levels"
    )
    ...
    hot_sectors: List[str] = Field(
        default_factory=list,
        description="Outperforming sectors"
    )
    cold_sectors: List[str] = Field(
        default_factory=list,
        description="Underperforming sectors"
    )
    rotating_sectors: List[str] = Field(
        default_factory=list,
        description="Sectors in rotation"
    )
    
    # Opportunities (IMMUTABLE)
    oversold_stocks: List[str] = Field(
        default_factory=list,
        description="Oversold stock symbols"
    )
    overbought_stocks: List[str] = Field(
        default_factory=list,
        description="Overbought stock symbols"
    )
    
    # Risk (IMMUTABLE)
    risk_level: RiskLevelV1 = Field(
        ...,
        description="Overall risk level for swing trading"
    )
...
class LongTermContextV1(BaseModel):
    ...
    economic_cycle: EconomicCycleV1 = Field(
        ...,
        description="Current economic cycle phase"
    )
    interest_rate_trend: Literal["rising", "falling", "stable"] = Field(
        ...,
        description="Interest rate trend"
    )
    inflation_trend: Literal["low", "moderate", "high"] = Field(
        ...,
        description="Inflation trend"
    )
    
    # Valuation Metrics (IMMUTABLE)
    nifty_pe: Optional[float] = Field(
        None,
        gt=0,
        description="Nifty 50 Price-to-Earnings ratio"
    )
    nifty_pb: Optional[float] = Field(
        None,
        gt=0,
        description="Nifty 50 Price-to-Book ratio"
    )
    ...
    emerging_themes: List[str] = Field(
        default_factory=list,
        description="Emerging investment themes"
    )
    declining_themes: List[str] = Field(
        default_factory=list,
        description="Declining investment themes"
    )
    
    # Allocation (IMMUTABLE)
    recommended_sector_weights: Dict[str, float] = Field(
        default_factory=dict,
        description="Recommended sector allocation percentages"
    )
    
    # Opportunities (IMMUTABLE)
    value_opportunities: List[str] = Field(
        default_factory=list,
        description="Value investment opportunities"
    )
    growth_opportunities: List[str] = Field(
        default_factory=list,
        description="Growth investment opportunities"
    )
    dividend_opportunities: List[str] = Field(
        default_factory=list,
        description="Dividend investment opportunities"
    )
    
    # Risk (IMMUTABLE)
    systemic_risk_level: RiskLevelV1 = Field(
        ...,
        description="Systemic market risk level"
    )
...
class DataQualityReportV1(BaseModel):
    ...
    overall_quality: DataQualityV1 = Field(
        ...,
    ...
    real_data_percentage: float = Field(
        ...
        ge=0.0,
        le=100.0,
        description="Percentage of real data (0-100)"
    )
    approximated_percentage: float = Field(
        ...,
        ge=0.0,
        le=100.0,
        description="Percentage of approximated data (0-100)"
    )
    fallback_percentage: float = Field(
        ...,
        ge=0.0,
        le=100.0,
        description="Percentage of fallback data (0-100)"
    )
...
class MarketContextResponseV1(BaseModel):
    ...
    primary_context: Optional[PrimaryMarketContextV1] = Field(
        None,
        description="Primary market context"
    )
    detailed_context: Optional[Dict[str, Any]] = Field(  # ADDITIVE: Detailed analysis
        None,
        description="Detailed market analysis (sectors, technicals, etc.)"
    )
    intraday_context: Optional[IntradayContextV1] = Field(
        None,
        description="Intraday trading context"
    )
    swing_context: Optional[SwingContextV1] = Field(
        None,
        description="Swing trading context"
    )
    long_term_context: Optional[LongTermContextV1] = Field(
        None,
        description="Long-term investment context"
    )

    # ML Features (ADDITIVE - OPTIONAL)
    ml_features: Optional[Dict[str, Any]] = Field(
        None,
        description="Flattened ML-ready features for machine learning (47+ features)"
    )
    
    # Data Quality (IMMUTABLE - OPTIONAL)
    data_quality: Optional[DataQualityReportV1] = Field(
        None,
    ...
    context_quality_score: Optional[float] = Field(
        None,
        ge=0.0, le=1.0,
        description="Overall quality score of context data (0.0-1.0)"
    )
    ...
    data_source_summary: Optional[Dict[str, int]] = Field(
        None,
        description="Summary of data sources: {real: N, approximated: N, fallback: N}"
    )
    ...
    context_quality_score: Optional[float] = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Overall context quality score (0.0 to 1.0)"
    )
    processing_time_ms: Optional[float] = Field(
        None,
        ge=0.0,
