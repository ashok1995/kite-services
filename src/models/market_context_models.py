...
class MarketContextRequest(BaseModel):
    ...
    symbols: Optional[List[str]] = Field(
        None,
        max_items=20,
        description="Specific symbols for contextual analysis"
    )
    
    # Data requirements
    data_freshness: DataFreshness = Field(
        default=DataFreshness.NEAR_REAL_TIME,
        description="Required data freshness"
    )
    
    # Feature toggles
    include_global_context: bool = Field(
        default=True,
        description="Include global market analysis"
    )
    
    include_sector_analysis: bool = Field(
        default=True,
        description="Include sector rotation and momentum"
    )
    
    include_technical_analysis: bool = Field(
        default=True,
        description="Include technical indicators and levels"
    )
    
    include_volatility_analysis: bool = Field(
        default=True,
        description="Include volatility and risk analysis"
    )
    
    include_sentiment_analysis: bool = Field(
        default=True,
        description="Include sentiment indicators"
    )
    
    include_trading_signals: bool = Field(
        default=False,
        description="Include trading signals (requires symbols)"
    )
    
    include_intraday_features: bool = Field(
        default=True,
        description="Include intraday-specific features"
    )
    ...
    secondary_timeframes: List[str] = Field(
        default=["5min", "1hour"],
        description="Additional timeframes for confluence"
    )
    
    # Strategy context
    strategy_type: Optional[str] = Field(
        None,
        description="Trading strategy type for contextual filtering"
    )
    ...
    historical_days: int = Field(
        ...
        ge=1,
        le=365,
        description="Days of historical context"
    )
    
    # Output preferences
    include_explanations: bool = Field(
        default=True,
        description="Include explanatory text and reasoning"
    )
    ...
    response_format: str = Field(
        default="detailed",
        description="Response format - minimal, standard, detailed"
    )
...
class GlobalMarketSnapshot(BaseModel):
    ...
    us_markets: Dict[str, Dict[str, Any]] = Field(
        ...
        description="US market indices and performance"
    )
    
    european_markets: Dict[str, Dict[str, Any]] = Field(
        default_factory=dict,
        description="European market indices and performance"
    )
    
    asian_markets: Dict[str, Dict[str, Any]] = Field(
        default_factory=dict,
        description="Asian market indices and performance"
    )
    ...
    correlations: Dict[str, Decimal] = Field(
        default_factory=dict,
        description="Cross-market correlation coefficients"
    )
...
class IndianMarketSnapshot(BaseModel):
    ...
    indices: Dict[str, Dict[str, Any]] = Field(
        default_factory=dict,
        description="Indian market indices"
    )
    ...
    institutional_flows: Dict[str, Decimal] = Field(
        default_factory=dict,
        description="FII/DII flows and activity"
    )
    ...
    session_characteristics: Dict[str, Any] = Field(
        default_factory=dict,
        description="Session-specific characteristics"
    )
...
class TechnicalSnapshot(BaseModel):
    ...
    timeframe_analysis: Dict[str, Dict[str, Any]] = Field(
        default_factory=dict,
        description="Analysis across multiple timeframes"
    )
    ...
    technical_indicators: Dict[str, Dict[str, Any]] = Field(
        default_factory=dict,
        description="Technical indicator values and signals"
    )
    
    # Momentum and trend
    trend_analysis: Dict[str, Any] = Field(
        default_factory=dict,
        description="Trend strength and momentum analysis"
    )
    
    # Volume analysis
    volume_analysis: Dict[str, Any] = Field(
        default_factory=dict,
        description="Volume patterns and analysis"
    )
...
class SentimentSnapshot(BaseModel):
    ...
    # Sentiment scores
    overall_sentiment_score: Decimal = Field(..., ge=-100, le=100, description="Overall sentiment score")
...
class TradingEnvironmentSnapshot(BaseModel):
    ...
    optimal_position_sizes: Dict[str, Decimal] = Field(
        default_factory=dict,
        description="Recommended position sizes by risk level"
    )
...
class MarketInsights(BaseModel):
    ...
    # Catalysts
    upcoming_catalysts: List[str] = Field(default_factory=list, description="Upcoming market catalysts")
...
class TradingRecommendations(BaseModel):
    ...
    strategies_to_avoid: List[str] = Field(
        default_factory=list,
        description="Strategies to avoid in current environment"
    )
    ...
    position_sizing_guidance: Dict[str, Any] = Field(
        default_factory=dict,
        description="Position sizing guidance by strategy"
    )
    
    # Execution guidance
    execution_recommendations: List[str] = Field(
        default_factory=list,
        description="Execution timing and method recommendations"
    )
    
    # Time horizon guidance
    optimal_time_horizons: Dict[str, str] = Field(
        default_factory=dict,
        description="Optimal time horizons for different strategies"
    )
...
class MarketContextResponse(BaseModel):
    ...
    symbol_analysis: Dict[str, SymbolSpecificContext] = Field(
        default_factory=dict,
        description="Symbol-specific contextual analysis"
    )
    
    # Market regime summary
    overall_market_regime: MarketRegime = Field(..., description="Overall market regime")
    regime_confidence: Decimal = Field(..., ge=0, le=100, description="Regime classification confidence")
    ...
    global_influence_score: Decimal = Field(
        ...
        ge=0, 
        le=100, 
        description="Global influence on Indian markets (0-100)"
    )
    
    correlation_breakdown: Dict[str, Decimal] = Field(
        default_factory=dict,
        description="Correlation with different global regions"
    )
    
    # Session and timing context
    optimal_trading_windows: List[str] = Field(
        default_factory=list,
        description="Optimal trading time windows for current session"
    )
    
    session_bias: str = Field(..., description="Current session trading bias")
    time_decay_factors: Dict[str, Any] = Field(
        default_factory=dict,
        description="Time decay factors for different strategies"
    )
    
    # Risk and opportunity matrix
    risk_opportunity_matrix: Dict[str, Dict[str, str]] = Field(
        default_factory=dict,
        description="Risk vs opportunity assessment by strategy"
    )
    
    # Performance metrics
    context_quality_score: Decimal = Field(..., ge=0, le=100, description="Overall context quality")
    data_coverage_score: Decimal = Field(..., ge=0, le=100, description="Data coverage completeness")
    ...
    data_sources_used: Dict[str, int] = Field(
        default_factory=dict,
        description="Data sources and data points used"
    )
...
class QuickMarketContextRequest(BaseModel):
    ...
    symbols: Optional[List[str]] = Field(None, max_items=5, description="Max 5 symbols for quick analysis")
...
class QuickMarketContextResponse(BaseModel):
    ...
    symbols: Dict[str, Dict[str, Any]] = Field(
        default_factory=dict,
        description="Quick symbol analysis"
    )
...
class MarketContextExamples:
    ...
    def quick_decision_context():
        ...
        return QuickMarketContextRequest(
