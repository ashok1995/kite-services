...
class GlobalMarketData(BaseModel):
    ...
    us_markets: Dict[str, Dict[str, float]] = Field(
        default_factory=dict, description="US market indices (S&P 500, NASDAQ, Dow)"
    )

    # European Markets
    european_markets: Dict[str, Dict[str, float]] = Field(
        default_factory=dict, description="European market indices (FTSE, DAX, CAC)"
    )

    # Asian Markets
    asian_markets: Dict[str, Dict[str, float]] = Field(
        default_factory=dict, description="Asian market indices (Nikkei, Hang Seng, Shanghai)"
    )
    ...
    correlations: Dict[str, Decimal] = Field(
        default_factory=dict, description="Cross-market correlations"
    )
...
class IndianMarketData(BaseModel):
    ...
    indices: Dict[str, Dict[str, float]] = Field(
        default_factory=dict, description="Indian market indices (NIFTY, BANK NIFTY, etc.)"
    )
...
class SectorData(BaseModel):
    ...
    sector_performance: Dict[str, Decimal] = Field(
        default_factory=dict, description="Sector performance percentages"
    )
    ...
    sector_breadth: Dict[str, int] = Field(
        default_factory=dict, description="Sector breadth (advancing vs declining)"
    )
...
class MarketContextData(BaseModel):
    ...
    global_influence: Decimal = Field(
...
class MarketContextRequest(BaseModel):
    ...
    include_institutional_data: bool = Field(
        default=True, description="Include institutional flows"
    )
