...
class TALibTechnicalAnalysisEngine:
    ...
    def analyze_price_data(
        ...
        if df.empty or len(df) < min_required:
            print(
                f"❌ Insufficient data for {horizon.value} TA-Lib analysis "
        ...
        try:
            ...
            _ = df["Volume"].values if "Volume" in df.columns else None
            ...
            print(
                f"✅ TA-Lib analysis complete: {len(trend_indicators)} trend, "
                f"{len(sentiment_indicators)} sentiment"
            )
        ...
        try:
            ...
            if not np.isnan(ema_short[-1]) and not np.isnan(ema_long[-1]):
                ...
                if prev_ema_short <= prev_ema_long and ema_short_val > ema_long_val:
                    ...
                    and current_price > ema_short_val
                    and ema_spread > 1.5
                ):
                    interpretation = "Strong Bullish"
                elif (
    ...
    def _calculate_talib_bollinger_bands(self, close: np.ndarray, periods: Dict) -> List[Trend]:
        ...
        try:
            bb_upper, bb_middle, bb_lower = talib.BBANDS(
                close, timeperiod=periods["bb_period"], nbdevup=2, nbdevdn=2, matype=0
            )
    ...
    def _calculate_talib_rsi(self, close: np.ndarray, periods: Dict) -> List[Sentiment]:
        ...
        try:
            ...
            if not np.isnan(rsi[-1]):
                ...
                indicators.append(
                    Sentiment(
                        indicator="TA-Lib RSI", value=float(rsi_val), interpretation=interpretation
                    )
                )
    ...
    def _calculate_talib_stochastic(
        self, high: np.ndarray, low: np.ndarray, close: np.ndarray, periods: Dict
    ) -> List[Sentiment]:
    ...
    def _calculate_talib_williams_r(
        self, high: np.ndarray, low: np.ndarray, close: np.ndarray, periods: Dict
    ) -> List[Sentiment]:
