...
class DatabaseManager:
    ...
    async def initialize_database(self):
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            await db.execute(
                "CREATE INDEX IF NOT EXISTS idx_pivot_symbol_date ON pivot_levels(symbol, date)"
            )
    ...
    async def calculate_and_store_pivot_levels(
        self, symbol: str, high: float, low: float, close: float, horizon: str
    ) -> PivotLevels:
        """
    ...
    async def get_pivot_levels(
        self, symbol: str, horizon: str, date_str: str = None
