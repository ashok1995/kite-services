...
from config.cache_config import CacheConfig, CacheTTL, CacheKeyPattern, CacheStrategy
...
class CacheService:
    ...
    def __init__(self, config: Optional[CacheConfig] = None):
        ...
        if not REDIS_AVAILABLE:
            self.logger.warning("Redis not available - caching disabled. Install: pip install redis")
            self.enabled = False
    ...
    async def initialize(self):
        ...
        try:
            ...
            self.logger.info(f"✅ Cache service initialized (Redis {self.config.host}:{self.config.port})")
    ...
    async def get_composite_intraday(self, timestamp_bucket: str) -> Optional[Dict]:
        ...
        key = self._make_key(
            CacheKeyPattern.COMPOSITE_INTRADAY,
            date=date,
            hour=hour,
            minute=minute
        )
    ...
    async def get_composite_swing(self, timestamp_bucket: str) -> Optional[Dict]:
        ...
        key = self._make_key(
            CacheKeyPattern.COMPOSITE_SWING,
            date=date,
            hour=hour
        )
        return await self.get(key)
