...
from models.enhanced_market_context_models import Horizon, Sentiment, Trend
...
class TechnicalAnalysisEngine:
    ...
    def analyze_price_data(
        self, df: pd.DataFrame, horizon: Horizon
    ) -> Tuple[List[Trend], List[Sentiment]]:
        """
        ...
        if df.empty or len(df) < min_required:
            print(
                f"❌ Insufficient data for {horizon.value} technical analysis "
                f"(need {min_required}+ bars, got {len(df)}) - Returning null"
            )
    ...
    def _calculate_moving_averages(self, df: pd.DataFrame, periods: Dict) -> List[Trend]:
        ...
        try:
            ...
            if sma_short is None or sma_long is None:
                print("❌ SMA calculation returned None")
                return indicators

            if sma_short.empty or sma_long.empty:
                print("❌ SMA calculation returned empty series")
                return indicators
            ...
            _ = abs(current_price - sma_short_val) / sma_short_val * 100
            ...
            if short_above_long and price_above_short and price_above_long and ma_spread > 2:
                ...
                not short_above_long
                and not price_above_short
            ...
            if ema_short is None or ema_long is None:
                print("❌ EMA calculation returned None")
                return indicators

            if ema_short.empty or ema_long.empty:
                print("❌ EMA calculation returned empty series")
                return indicators
            ...
            if prev_ema_short <= prev_ema_long and ema_short_val > ema_long_val:
                ...
                ema_short_val > ema_long_val and current_price > ema_short_val and ema_spread > 1.5
            ):
            ...
            indicators.append(
                Trend(
                    ...
                    value={
                        ...
                        "crossover_detected": prev_ema_short <= prev_ema_long
                        and ema_short_val > ema_long_val
                        or prev_ema_short >= prev_ema_long
                        and ema_short_val < ema_long_val,
                    },
                    interpretation=interpretation,
                )
    ...
    def _calculate_bollinger_bands(self, df: pd.DataFrame, periods: Dict) -> List[Trend]:
        ...
        try:
            ...
            # Create DataFrame similar to pandas-ta output
            bb_data = pd.DataFrame({"BBL": bb_lower, "BBM": sma, "BBU": bb_upper})
    ...
    def _calculate_rsi(self, df: pd.DataFrame, periods: Dict) -> List[Sentiment]:
        ...
        try:
            ...
            if not rsi.empty:
                ...
                indicators.append(
                    Sentiment(indicator="RSI", value=float(rsi_val), interpretation=interpretation)
                )
    ...
    def _calculate_stochastic(self, df: pd.DataFrame, periods: Dict) -> List[Sentiment]:
        ...
        try:
            ...
            # Create DataFrame similar to pandas-ta output
            stoch_data = pd.DataFrame({"STOCHk": k_percent, "STOCHd": d_percent})
    ...
    def _calculate_williams_r(self, df: pd.DataFrame, periods: Dict) -> List[Sentiment]:
        ...
        try:
            ...
            if not williams_r.empty:
                ...
                indicators.append(
                    Sentiment(
                        indicator="Williams %R", value=float(wr_val), interpretation=interpretation
                    )
                )
