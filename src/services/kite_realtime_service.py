...
class KiteRealTimeService:
    ...
    def _calculate_technical_indicators(self, symbol: str):
        ...
        try:
            if symbol not in self.historical_data or len(self.historical_data[symbol]) < 20:
                logger.debug(f"Not enough historical data for {symbol}: {len(self.historical_data.get(symbol, []))}")
                return
            ...
            price_momentum = ((prices[-1] - prices[-5]) / prices[-5] * 100) if len(prices) >= 5 else 0
    ...
    def _determine_signal(self, indicators: TechnicalIndicators, overall_score: float) -> tuple:
        ...
        if (overall_score > 80 and 
            ...
            indicators.price_momentum > 1):
            signal = "buy"
            confidence = min(0.9, overall_score / 100)
        
        # Strong sell signals
        elif (overall_score < 30 and 
        ...
        # Weak buy signals
        elif (overall_score > 60 and 
              indicators.rsi < 80 and 
              indicators.macd > indicators.macd_signal):
            signal = "buy"
            confidence = min(0.7, overall_score / 100)
        
        # Weak sell signals
        elif (overall_score < 40 and 
              indicators.rsi > 20 and 
              indicators.macd < indicators.macd_signal):
            signal = "sell"
    ...
    def _calculate_targets(self, entry_price: float, indicators: TechnicalIndicators, signal: str) -> tuple:
        ...
        for symbol in mock_symbols:
            if symbol in base_prices:
                ...
                for i in range(50):  # Generate 50 historical points
                    ...
                    mock_tick = {
                        'instrument_token': self._get_mock_instruments()[symbol]['instrument_token'],
...
def main():
    ...
    def on_analysis_update(rankings):
        ...
        for ranking in rankings[:5]:  # Top 5
            print(f"{ranking.rank}. {ranking.symbol}: {ranking.overall_signal} (score: {ranking.score:.2f})")
