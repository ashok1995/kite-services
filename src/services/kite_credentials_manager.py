...
class KiteCredentialsManager:
    ...
    def save_credentials(
        self, credentials: KiteCredentials, file_path: Optional[str] = None
    ) -> bool:
    ...
    def validate_credentials(
        self, credentials: Optional[KiteCredentials] = None
    ) -> Tuple[bool, str]:
        ...
        try:
            ...
            if profile and profile.get("user_id"):
                logger.info(
                    f"✅ Credentials validated for user: {profile.get('user_name', 'Unknown')}"
                )
    ...
    def create_sample_credentials_file(self, file_path: str = "access_token.json") -> bool:
        ...
        try:
            sample_credentials = KiteCredentials(  # pragma: allowlist secret
...
if __name__ == "__main__":
    ...
    if credentials.is_valid():
        ...
        if not is_valid:
            print(
                "❌ Credentials are invalid. Please check your API key, secret, and access token."
            )
    else:
