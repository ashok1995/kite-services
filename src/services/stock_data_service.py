...
from models.data_models import (
    RealTimeStockData, RealTimeRequest, RealTimeResponse,
    HistoricalStockData, HistoricalRequest, HistoricalResponse,
    Candle, Exchange, Interval, DataError
)
...
class StockDataService:
            ...
            for symbol in request.symbols:
                try:
                    ...
                    stock_data = self._build_historical_stock_data(
                        symbol, request.exchange, instrument_token, 
                        request.interval, from_date, to_date, historical_data
                    )
    ...
    def _build_real_time_stock_data(
        ...
        change_percent = Decimal(str(change / close_price * 100)) if close_price and close_price > 0 else Decimal('0')
        ...
        return RealTimeStockData(
            ...
            average_price=Decimal(str(quote.get('average_price', 0))) if quote.get('average_price') else None,
            ...
            upper_circuit=Decimal(str(quote.get('upper_circuit_limit', 0))) if quote.get('upper_circuit_limit') else None,
            lower_circuit=Decimal(str(quote.get('lower_circuit_limit', 0))) if quote.get('lower_circuit_limit') else None,
