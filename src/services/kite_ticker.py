...
class KiteTickerService:
    ...
    async def _load_credentials(self) -> None:
        ...
        try:
            ...
            if not self.config.api_key or not self.config.access_token:
                self.logger.warning(
                    "API key or access token not found - service will use mock data"
                )
                return
    ...
    async def _initialize_ticker(self) -> None:
        ...
        try:
            ...
            self.kite_ticker = KiteTicker(
                api_key=self.config.api_key, access_token=self.config.access_token
            )
    ...
    async def _resubscribe_tokens(self) -> None:
        ...
        try:
            if self.subscribed_tokens:
                ...
                self.kite_ticker.set_mode(self.kite_ticker.MODE_QUOTE, self.subscribed_tokens)
    ...
    async def subscribe(self, instrument_tokens: List[int]) -> None:
        ...
        try:
            if not self.kite_ticker:
                self.logger.warning(
                    "Kite ticker not available - storing tokens for later subscription"
                )
        ...
        try:
            if not self.kite_ticker:
                ...
                self.subscribed_tokens = [
                    token for token in self.subscribed_tokens if token not in instrument_tokens
                ]
                return
