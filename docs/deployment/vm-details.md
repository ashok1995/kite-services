...
```bash
TEST_BASE_URL=http://35.232.205.155:8179 \
...
2. Get login URL: `curl -s http://35.232.205.155:8179/api/auth/login-url | jq .`
3. Open URL in browser, log in, copy `request_token` from redirect.
4. Exchange:

   ```bash
   curl -X POST http://35.232.205.155:8179/api/auth/login \
     -H "Content-Type: application/json" \
     -d '{"request_token":"<TOKEN>"}' | jq .
   ```

5. Verify: `curl -s http://35.232.205.155:8179/api/auth/status | jq .`

### Health check

```bash
curl -s http://35.232.205.155:8179/health | jq .
```
