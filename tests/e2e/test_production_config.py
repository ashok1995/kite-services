...
class TestProductionConfig:
    ...
    def test_settings_load_with_production_env(self):
        ...
        for key in list(os.environ.keys()):
            if key.startswith(("KITE_", "CORS_", "REDIS_", "DATABASE_", "SERVICE_", "ENVIRONMENT", "DEBUG", "LOG_LEVEL")):
                env_backup[key] = os.environ[key]
                if key in test_env:
    ...
    @pytest.mark.parametrize("cors_value,expected", [
        ('["*"]', ["*"]),  # JSON array string
        ('["http://localhost:3000","https://example.com"]', ["http://localhost:3000", "https://example.com"]),
        ('*', ["*"]),  # Single string (should be converted to list)
        ('http://localhost:3000,https://example.com', ["http://localhost:3000", "https://example.com"]),  # Comma-separated
    ...
    def test_cors_origins_parsing(self, cors_value, expected):
        ...
        try:
            ...
            if isinstance(cors_origins, list):
                assert len(cors_origins) > 0, f"CORS origins should not be empty, got: {cors_origins}"
                # Check that expected values are present (order may vary)
                if expected != ["*"]:
                    for exp in expected:
                        assert exp in cors_origins or "*" in cors_origins, \
    ...
    def test_settings_with_missing_optional_fields(self):
        ...
        env_backup = {}
        optional_keys = [
            "REDIS_PASSWORD", "CORS_ORIGINS", "CORS_METHODS", "CORS_HEADERS",
    ...
    def test_settings_with_invalid_types(self):
        ...
        try:
            ...
            try:
                ...
                assert "validation" in str(e).lower() or "type" in str(e).lower() or "invalid" in str(e).lower(), \
    ...
    def test_app_initializes_with_production_config(self):
        ...
        prod_env = {
            ...
            "KITE_ACCESS_TOKEN": "test_token_456",
            "DATABASE_URL": "sqlite+aiosqlite:///:memory:",
            "REDIS_ENABLED": "false",  # Disable Redis for test
        }
        
        for key in list(os.environ.keys()):
            if any(key.startswith(prefix) for prefix in ["KITE_", "CORS_", "REDIS_", "DATABASE_", "SERVICE_", "ENVIRONMENT", "DEBUG", "LOG_LEVEL"]):
                env_backup[key] = os.environ[key]
