...
class TestEnhancedContextIntegration:
    ...
    def test_primary_context_only(self, client):
        ...
        assert data["success"] is True, "Should be successful"
    ...
    def test_data_quality_reporting(self, client):
        ...
        response = client.post("/api/analysis/context/enhanced", json={"include_primary": True})
        ...
        assert (
            real_count + approx_count + fallback_count == total_count
        ...
        print(
            f"\n✅ Data Quality: {real_pct}% real, {approx_pct}% calculated, "
            f"{fallback_pct}% fallback"
        )
    ...
    def test_indian_markets_real_data(self, client):
        ...
        if nifty_change != 0:
            ...
            quality = data["data_quality"]
            sources = quality["data_sources"]
            has_real_indian = any("Indian markets: REAL" in src for src in sources)
    ...
    def test_performance(self, client):
        ...
        ), f"Primary context should respond in <5s, got {response_time_ms:.0f}ms"
...
class TestDataQualityValidationAPI:
    ...
    def test_data_source_labeling(self, client):
        ...
        for source in sources:
            assert any(
                keyword in source for keyword in ["REAL", "CALCULATED", "APPROXIMATED"]
    ...
    def test_recommendations(self, client):
        ...
        if quality in ["LOW", "MEDIUM"]:
            ...
            ), "Should have recommendations when quality is low/medium"
