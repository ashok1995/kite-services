...
from core.kite_client import KiteClient  # noqa: E402
...
class TestKiteConnectIntegration:
    ...
    async def test_quote_api_nifty(self, kite_client):
        ...
        print(f"\n✅ Nifty Quote: {nifty['last_price']} ({nifty['net_change']})")  # noqa: G004
    ...
    async def test_quote_api_multiple_symbols(self, kite_client):
        ...
        print(f"\n✅ Received {len(quotes)} quotes for multiple symbols")  # noqa: G004
...
class TestDataQualityValidation:
    ...
    def test_validate_price_range(self):
        ...
        # Valid price
        assert self._validate_price(25000.50) is True

        # Invalid prices
        assert self._validate_price(0) is False
        assert self._validate_price(-100) is False
        assert self._validate_price(None) is False
    ...
    def test_validate_change_consistency(self):
        ...
        assert (
            abs(net_change - expected_net_change) < 0.1
    ...
    def test_calculate_data_quality_score(self):
        ...
        # Missing data
        incomplete_data = {"last_price": 25000, "net_change": -50}
