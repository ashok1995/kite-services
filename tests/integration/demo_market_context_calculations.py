...
async def demo_market_strength_calculation():
    ...
    if len(leading_sectors) > len(lagging_sectors):
        sector_score = 15
        sector_reason = f"More leaders ({len(leading_sectors)}) than laggards ({len(lagging_sectors)})"
    elif len(lagging_sectors) > len(leading_sectors):
        sector_score = -15
        sector_reason = f"More laggards ({len(lagging_sectors)}) than leaders ({len(leading_sectors)})"
    else:
        sector_score = 0
...
async def demo_global_influence_calculation():
    ...
    for region, change in regional_changes.items():
        ...
        print(f"   {region} Component: {change:+.2f}% × {correlation:.2f} × {weight:.1f} = {influence_component:+.3f}")
