...
def test_data_models():
    ...
    try:
        ...
        from src.models.data_models import (
            ...
            HistoricalRequest, HistoricalResponse, HistoricalStockData,
            Candle, Exchange, Interval, DataExamples
        )
    ...
    try:
        ...
        try:
            response = requests.post(
                f"{base_url}/api/stock-data/real-time",
            ...
            if response.status_code == 200:
                ...
                if real_time_data.get('stocks'):
                    ...
                    print(f"      Sample Data: {sample_stock.get('symbol', 'N/A')} @ ₹{sample_stock.get('last_price', 'N/A')}")
        ...
        try:
            response = requests.post(
                f"{base_url}/api/stock-data/historical",
                json=historical_request,
                timeout=15
            )
            if response.status_code == 200:
                historical_data = response.json()
                print(f"   ✅ Historical Data: {historical_data.get('processing_time_ms', 'N/A')}ms")
                ...
                if historical_data.get('stocks'):
                    ...
                    if candles:
                        ...
                        print(f"      Latest: O:{latest_candle.get('open')} H:{latest_candle.get('high')} L:{latest_candle.get('low')} C:{latest_candle.get('close')}")
